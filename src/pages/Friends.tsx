...
const UserListCard: React.FC<UserListCardProps> = ({
  ...
  <div className="flex flex-row items-center justify-between p-4 bg-muted/30 rounded-lg">
    ...
    <Button
      ...
      className="ml-2 whitespace-nowrap text-xs sm:text-sm"
    >
      {isFriend ? (
...
const Friends = () => {
  ...
  const pendingRequestIds = useMemo(
    () => new Set(pendingRequests?.map((r) => r.sender.id)),
    [pendingRequests]
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 sm:pt-24 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
        ...
        <Card className="border-0 bg-card">
          ...
          <CardContent className="pb-5 sm:pb-6">
            {friends && friends.length > 0 ? (
              ...
              <p className="text-muted-foreground text-center text-sm sm:text-base py-4">
        ...
        {/* Right column: pending + search */}
        <div className="space-y-6 sm:space-y-8">
          {pendingRequests && pendingRequests.length > 0 && (
            <Card className="border-0 bg-card border-coral/30">
              <CardHeader className="pb-3 sm:pb-4">
                <CardTitle className="text-lg sm:text-xl text-coral">
                  Pending Requests
                </CardTitle>
                <CardDescription className="text-xs sm:text-sm">
                  These users want to add you as a friend.
                </CardDescription>
              </CardHeader>
            ...
            <CardContent className="space-y-4 pb-5 sm:pb-6">
              ...
              <div className="space-y-2">
                {isSearching && (
                  <div className="flex items-center space-x-4 p-4 bg-muted/30 rounded-lg">
                    ...
                    <div className="space-y-2 w-full">
                      ...
                      <Skeleton className="h-3 sm:h-4 w-[100px]" />
                    </div>
                  </div>
                )}
                ...
                {!isSearching &&
                  ...
                  searchResults.length === 0 && (
                    <p className="text-muted-foreground text-center pt-3 text-xs sm:text-sm">
